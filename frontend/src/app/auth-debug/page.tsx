...
export default function AuthDebugPage() {
  ...
  const testApiCall = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/books?page=1&limit=5', {
        headers: {
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-4xl mx-auto space-y-6">
        <h1 className="text-3xl font-bold text-center">Authentication Debug Tool</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <div className="text-sm">
                ...
                <p><strong>Has Auth Token:</strong> {debugInfo.hasAuthToken ? '✅' : '❌'}</p>
                <p><strong>Has User:</strong> {debugInfo.hasUser ? '✅' : '❌'}</p>
                <p><strong>Token Length:</strong> {debugInfo.tokenLength}</p>
                <p><strong>Auth Token Length:</strong> {debugInfo.authTokenLength}</p>
                <p><strong>Token Preview:</strong> {debugInfo.tokenPreview}</p>
                <p><strong>Auth Token Preview:</strong> {debugInfo.authTokenPreview}</p>
                {debugInfo.userInfo && (
                  <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={checkAdminSetup} className="w-full" variant="outline">
                Check Admin Setup
              </Button>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <ol className="list-decimal list-inside space-y-2 text-sm">
                ...
                <li>If no admin exists, go to <a href="/admin-setup" className="text-blue-600 underline">/admin-setup</a></li>
